...
import { useRecruteEndDateContext, useRecruteStartDateContext, useRecruteTitleContext, useRecruteUseLanguageContext } from '../../providers/RecrutePostProvider'
	...
	return (
		...
		<RecruteNavigatorBackGround>
			...
			<Grid container>
				...
				<Grid xs={6}>
					<InputFieldWrapper>
						<Button variant="outlined" onClick={onClickOpenModal} >この条件で募集する</Button>
					</InputFieldWrapper>
				</Grid>
			</Grid>
			<Modal
      ...
      >
        <Box sx={style}>
          <Typography id="modal-modal-title" variant="h6" component="h2">
						この条件で募集しますか？<br/>
						title: {recruteTitle}<br />
						useLangage: {recruteUseLanguage} <br/>
						startDate: {moment(recruteStartDate).format("MMMM Do, h:mm a").toString()}<br />
						endDate: {moment(recruteEndDate).format("MMMM Do, h:mm a").toString()}
          </Typography>
